// ConnectivityGate.jsx — blocks the Customer app if offline or location denied
...
export default function ConnectivityGate({ children }) {
  ...
  const [locState, setLocState] = useState("checking"); // checking | granted | denied
  ...
  useEffect(() => {
    ...
    if (navigator.permissions) {
      navigator.permissions.query({ name: "geolocation" }).then((r) => {
        ...
        else                            requestPos(); // "prompt" → ask user
        r.onchange = () => {
          if (r.state === "granted")     setLocState("granted");
          else if (r.state === "denied") setLocState("denied");
        };
